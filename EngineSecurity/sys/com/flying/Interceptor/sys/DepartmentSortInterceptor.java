...
public class DepartmentSortInterceptor extends AbstractInterceptor {
	...
	public void before(EngineParameter ep) throws Exception {
		...
		if(!oldNodeId.equals(newNodeId)){
			EngineParameter selfEp = new EngineParameter("T_SYS_DEPARTMENT.update");
			selfEp.putParam("DEPARTMENT_ID", nodeId);
		...
		if(!"".equals(beforeNodeId)){
			...
			for(int i = 0;i<beforeNodeArray.length;i++){
				EngineParameter selfEp = new EngineParameter("T_SYS_DEPARTMENT.updateSort");
				selfEp.putParam("DEPARTMENT_ID", beforeNodeArray[i]);
				selfEp.putParam("ORDER_ID", sortNum - beforeNodeArray.length + i);
				listEp.add(selfEp);
			}
		}
		...
		if(!"".equals(afterNodeId)){
			...
			for(int i = 0;i<afterNodeArray.length;i++){
				EngineParameter selfEp = new EngineParameter("T_SYS_DEPARTMENT.updateSort");
				selfEp.putParam("DEPARTMENT_ID", afterNodeArray[i]);
				selfEp.putParam("ORDER_ID", sortNum + i + 1);
				listEp.add(selfEp);
			}
		}
