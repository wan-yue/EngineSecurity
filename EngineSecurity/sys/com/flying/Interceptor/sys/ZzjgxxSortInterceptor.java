...
public class ZzjgxxSortInterceptor extends AbstractInterceptor {
	...
	public void before(EngineParameter ep) throws Exception {
		...
		if(!oldNodeId.equals(newNodeId)){
			EngineParameter selfEp = new EngineParameter("ZZJGXX.update");
			selfEp.putParam("ZZJGDM", nodeId);
			selfEp.putParam("SJZZJGDM", newNodeId);
		...
		if(!"".equals(beforeNodeId)){
			...
			for(int i = 0;i<beforeNodeArray.length;i++){
				...
				selfEp.putParam("ZZJGDM", beforeNodeArray[i]);
				selfEp.putParam("PXH", sortNum - beforeNodeArray.length + i);
				listEp.add(selfEp);
			}
		}
		...
		if(!"".equals(afterNodeId)){
			...
			for(int i = 0;i<afterNodeArray.length;i++){
				EngineParameter selfEp = new EngineParameter("T_SYS_DEPARTMENT.updateSort");
				selfEp.putParam("ZZJGDM", afterNodeArray[i]);
				selfEp.putParam("PXH", sortNum + i + 1);
				listEp.add(selfEp);
			}
		}
