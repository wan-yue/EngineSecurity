...
Ext.flying.UploadDialog = Ext.extend(Ext.Window, {
	grid : null,//文件表格
	swfupload : null,//上传控件
	progressBar : null,//进度条
	progressInfo : null,//进度信息
	uploadInfoPanel : null,//上传信息区
	constructor : function(config) {
		Ext.flying.UploadDialog.parent = config.scope;	//this指针得不到传递的scope属性
		...
		this.grid.on('render',function(){
			this.grid.getBottomToolbar().add(this.progressBar);
			...
			var settings = {
				...
				file_dialog_complete_handler : this.onFileDialogComplete,//文件选择成功时触发
				upload_start_handler : this.onUploadStart,
				upload_progress_handler : this.onUploadProgress,
				upload_error_handler : this.onUploadError,	//服务端返回了非200的HTTP状态时触发的处理函数
				upload_success_handler : this.onUploadSuccess, //**服务端返回了200的HTTP状态时，触发此事件
		...
		Ext.flying.UploadDialog.superclass.constructor.call(this, Ext.applyIf(config || {},{
			title : '文件上传',
			closeAction : 'close',
			layout : 'border',
	...
	onFileQueued : function(file){
		...
		thiz.grid.getStore().add(new Ext.flying.UploadDialog.FileRecord({
			fileId : file.id,
			fileName : file.name,
			fileSize : file.size,
			fileType : file.type,
...
//			}
	...
	onUploadSuccess : function(file, serverData){
		...
		if(data.success){
			...
			thiz.updateProgressInfo();
			//刷新列表
			Ext.flying.UploadDialog.parent.store.reload();
		}else{	//上传验证失败，给出提示信息
			Ext.Msg.show({
				title : '错误提示',
				msg : data.msg,
...
Ext.flying.UploadDialog.FileRecord = Ext.data.Record.create([
	{name : 'fileId'},
  	{name : 'fileName'},
  	{name : 'fileSize'},
  	{name : 'fileType'},
