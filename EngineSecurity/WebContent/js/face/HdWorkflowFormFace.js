function HdWorkflowFormFace(configs) {
	/** 全局属性区 */
	...
	this.uploadAction = "common.action?command=workflow.hdPublish"; // 上传请求的url
	...
	this.handleColumn = function() {// 列处理方法
		...
		for ( var m = 0; m < me.columns.length; m++) {// 构建表格列
			...
			if(me.columns[m]["isFlow"] != null && me.columns[m]["isFlow"] == false){
				continue;
			}
	...
	this.handleButton = function() {// 操作处理方法
		...
		for ( var i = 0; i < me.btns.length; i++) {// 遍历
			if(!(me.btns[i]["isFlow"] != null && me.btns[i]["isFlow"] == false)){
		...
		handler : function(btn) {
			...
			if (me.opType == 'add') {
				...
				for ( var i = 0; i < me.columns.length; i++) {// 遍历列对象
					...
					if(me.columns[i].xtype != null && me.columns[i].xtype == "datefield"){
						model.data[name] = me.form.getForm().findField(name).getValue().format('Y-m-d');
					}else if(me.columns[i].xtype != null && me.columns[i].xtype == "combo"){
						var showName = me.columns[i].displayField;
						model.data[name] = me.form.getForm().findField(name).getValue();
						model.data[showName] = me.form.getForm().findField(name).getRawValue();
					}else{
						model.data[name] = me.form.getForm().findField(name).getValue();
					}
				}
				...
				me.grid.stopEditing();
		        var count = me.ds.getCount();
		        me.ds.insert(count, model);
				...
				for ( var i = 0; i < me.columns.length; i++) {// 遍历列对象
					...
					if(me.columns[i].xtype != null && me.columns[i].xtype == "datefield"){
						row.data[name] = me.form.getForm().findField(name).getValue().format('Y-m-d');
					}else if(me.columns[i].xtype != null && me.columns[i].xtype == "combo"){
						var showName = me.columns[i].displayField;
						row.data[name] = me.form.getForm().findField(name).getValue();
						row.data[name] = me.form.getForm().findField(name).getRawValue();
					}else{
						row.data[name] = me.form.getForm().findField(name).getValue();
					}
				}
				...
				Ext.Ajax.request({
					...
					success : function(response, opts) {	
						...
						if(resultObj.data.length == 0){
							...
							me.auditUserId = resultObj.data[0]["USER_ID"];
						}else if(resultObj.data.length > 1){
											...
											if(checked){
												Ext.Msg.confirm('下一步审核人', '确定让'+radio.boxLabel+'审核吗?', function(btn) {
													if (btn == 'yes') {
														...
														me.params["businessTable"] = me.en;
														me.params["businessArray"] = Ext.util.JSON.encode(arr);
														me.params["AUDIT_USER_ID"] = me.auditUserId;
