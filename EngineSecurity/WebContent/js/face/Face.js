...
//构造函数
Ext.flying.Face = function(configs){
	//this.configs = {};
	
	Ext.apply(this,configs);
};

Ext.apply(Ext.flying.Face.prototype,{
	/** 全局属性区 */
	configs : this.configs || {},// 配置属性
	...
	formFileUpload : false,//表单是否含有上传控件
	
	leftArray : new Array(),// 表单左侧数组

	rightArray : new Array(),// 表单右侧数组

	formArray : new Array(),// 表单数组
	
	modelArray : new Array(),// 实体数组

	buttonArray : new Array(),// 操作数组

	isSearch : true,// 是否需要查询功能

	searchArray : new Array(),// 查询数据的数组

	cmArray : new Array(),// 表格列数组
	
	/** 生命周期中的方法*/
	beforeAddShow : Ext.emptyFn,//添加页面显示之前方法
	
	afterAddShow : function(){//添加页面显示之后方法
		this.kindeditorInit();
	},
	
	beforeAddSave : Ext.emptyFn,//添加保存之前方法
	
	afterAddSave : Ext.emptyFn,//添加保存之后方法
	
	beforeModifyShow : Ext.emptyFn,//修改页面显示之前方法
	
	afterModifyShow : function(){//修改页面显示之后方法
		this.kindeditorInit();
	},
	
	beforeModifySave : Ext.emptyFn,//修改保存之前方法
	
	afterModifySave : Ext.emptyFn,//修改保存之后方法
	
	beforeDeleteSave : Ext.emptyFn,//删除保存之前方法
	
	afterDeleteSave : Ext.emptyFn,//删除保存之后方法
	
	reset : function(){//初始化方法
		this.params = {};
		this.tabs.removeAll();
	},

	addSuccess : function() {//添加成功方法
		var me = this;
	...
	gridRowClick : function() {//初始化表格行单击事件
		Ext.fcache.obj = this;
	},
	
	gridDbRowClick : Ext.emptyFn,//初始化表格双击事件
	
	gridRender : Ext.emptyFn,//表格渲染事件
	...
	/** 表单对象包装方法*/
	formPack : function(formObj) {// 表单项包装方法
		var me = this;
	...
	clickAdd : function() {
		...
		me.tabs.insert(0,me.formPanel);
		me.tabs.setActiveTab(me.formPanel);
	...
	clickShow : function(button,event) {
		var me = this;
		...
		if(me[button["linkName"]] && me[button["linkName"]]["center"])
		{
			...
			me.tabs.insert(0,me[button["linkName"]]["center"]);
			me.tabs.setActiveTab(me[button["linkName"]]["center"]);
		}
			...
			if (row) {
				Ext.Msg.confirm(b.text, '确定'+b.text+'?', function(btn) {
					if (btn == 'yes') {
						...
						if(me.opType == "modify"){
							me.tabs.remove(me.formPanel,false);//关闭修改页面
						}
	...
	store_search : new Ext.data.SimpleStore({//查询数据集合
		fields : [ 'field', 'fieldValue' ],
		data : []
	}),
	
	combo_search : new Ext.form.ComboBox({//查询下来列表
		xtype : 'combo',
	...
	txt_search : new Ext.form.TextField({//查询文本框
	...
	constructor : this.Constructor(), // 执行构造方法，初始化对象
	
	/** 页面跳转区 */
	dispatch : function(resultObject) {// type=1:显示表单；type=2:详情；type=3:审核。调整到表单申请页面
		// 从缓存中请求
...
//													}
	...
	btn_cancel : new Ext.Button({// 取消按钮
		text : '取消',
		iconCls : 'remove',
		scope:this,
		handler : function() {			
			...
			me.tabs.remove(me.formPanel,false);
	...
	/** grid构建区域 */
	Model : Ext.data.Record.create(this.modelArray),// 用户对象

	ds : new Ext.data.Store({// 数据源
		autoLoad : true,
		remoteSort : true,
	...
	pagingBar : new Ext.PagingToolbar({// 分页工具栏
		pageSize : this.pageSize,
		store : this.ds,
		displayInfo : false
	}),

	toolbar : new Ext.FlyingToolbar({// 操作工具栏
		items : this.buttonArray
	}),

	grid : new Ext.grid.FlyingGridPanel({// 列表
		iconCls : 'icon-grid',
		border : false,
	...
	tabs : new Ext.FlyingTabPanel({//tabs
    	region:'south',
    	border : false,
    	resizeTabs : true,
    	autoDestroy : false,
